...
const PIECE_FLAG_BLACK: u8 = 1;
const PIECE_FLAG_WHITE: u8 = 2;
const PIECE_FLAG_CROWN: u8 = 4;

impl Into<i32> for GamePiece {
    fn into(self) -> i32 {
        let mut val: u8 = 0;
        if self.color == PieceColor::Black {
            val += PIECE_FLAG_BLACK;
        } else if self.color == PieceColor::White {
            val += PIECE_FLAG_WHITE;
        }
        if self.crowned {
            val += PIECE_FLAG_CROWN;
        }
        val as i32
    }
}
...
pub extern "C" fn move_piece(fx: i32, fy: i32, tx: i32, ty: i32) -> i32 {
    ...
    match res {
        Ok(mr) => {
            ...
            if mr.crowned {
                unsafe { notify_piececrowned(tx, ty) }
            }
            1
        }
        Err(_) => 0,
